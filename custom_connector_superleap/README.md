...
This tutorial also includes an example of Superleap connector that you can use immediately in AppFlow and test custom connector functionalities. This tutorial includes details about Superleap connector built using aws-appflow-custom-connector-python-sdk module.

## SuperleapExampleConnector:

A sample connector built using aws-appflow-custom-connector-python-sdk module to interact with Superleap APIs using Http REST APIs in Amazon AppFlow with following capabilities,

* Accessing Superleap:
    * Use OAuth2 credentials
* Connector as Source:
    * Fetch given data set of record IDs or apply query filtering to fetch data from Superleap.
    * Fetch all possible records using pagination token
    * Incrementally pull delta from Superleap
* Connector as destination:
    * Write a batch of records to Superleap from Amazon AppFlow

This sample connector can be used as with any other compatible connector in AppFlow to transfer data from any other connector to Superleap and Superleap to any other connector. Additionally, this same connection can be used as both source and destination to transfer within Superleap. A connector should have ConfigurationHandler, MetadataHandler and RecordHandler to make it use within Amazon AppFlow.
...
```python
AUTH_URL = 'https://login.superleap.com/services/oauth2/authorize'
LOGIN_URL = 'https://login.superleap.com/services/oauth2/token'
REFRESH_URL = 'https://login.superleap.com/services/oauth2/token'
REDIRECT_URL = 'https://login.superleap.com'
...
* Supported API versions: v51.0
    * Today this sample connector uses Superleap v51.0 version of the APIs. In future we may add more supported API versions. When there are more API versions, AppFlow users can choose a particular version and execute their flows.
...
class SuperleapConfigurationHandler(ConfigurationHandler):
    ...
    def describe_connector_configuration(self, request: requests.DescribeConnectorConfigurationRequest) -> \
        ...
        api_version_setting = settings.ConnectorRuntimeSetting(key=API_VERSION_KEY,
                                                               ...
                                                               .String,
                                                               required=True,
                                                               label='Superleap API version',
                                                               description='Superleap API version to use.',
                                                               scope=settings.ConnectorRuntimeSettingScope
                                                               .CONNECTOR_PROFILE)
...
class SuperleapMetadataHandler(MetadataHandler):
    ...
    def list_entities(self, request: requests.ListEntitiesRequest) -> responses.ListEntitiesResponse:
        ...
        """to Superleap and convert the response to the Entity format."""

    def describe_entity(self, request: requests.DescribeEntityRequest) -> responses.DescribeEntityResponse:
        """Using the credentials provided in the request, invoke metadata API calls"""
        """to Superleap and convert the response to the EntityDefinition format."""
```

### RecordHandler

AppFlow’s Flow execution for a custom connector mostly depends on the implementation of of this RecordHandler interface. This is where we expect customers to implement the core functionality of making requests source/destination application in order to get/post data. In the Superleap example we have implemented three methods of RecordHandler. RetrieveData is used to lookup records against a batch of id’s from the source connector. Input request object for this method contains fieldname along with batch of id’s corresponding to that filed .  QueryData method is used when users is interested in querying the under laying application by using a filter query.  The filter query given by the user should follow specific AppFlow DSL and AppFlow SDK will help in parsing this filter expression. WriteData method is used to write a batch of records to destination connector. Request object which is passed as a parameter this method will have a list objects which have to be written to destination application. In the given Superleap example we process each record present in the list and write it to Saleforce by building an appropriate write query.
```python
class SuperleapRecordHandler(RecordHandler):
    ...
    def retrieve_data(self, request: requests.RetrieveDataRequest) -> responses.RetrieveDataResponse:
        """Retrieves the batch of records against a set of identifiers from Superleap"""
        """Build a superleap request URI with the list of identifiers and make request"""
    ...
    def write_data(self, request: requests.WriteDataRequest) -> responses.WriteDataResponse:
        """Writes batch of records to the Superleap"""
        """Request object contains batch of records to be written to superleap."""
        """build a request object to write records to superleap using the customer"""
        """credentials provided in the request."""
    def query_data(self, request: requests.QueryDataRequest) -> responses.QueryDataResponse:
        """Queries the data from Superleap  against the supplied filter conditions."""
        """Request object contains a filter expression which is used the saleforce request URI."""
```

### Connected App configuration in Superleap for Private Beta:

Follow these instructions to create a connected app in Superleap if you haven't done so already.
*To create a global connected app in Superleap*

1. Log in to Superleap with an account that has administrator rights, and go to *Setup*.
...
3. Choose *New Connected App* in the upper-right corner, and enter the following information for your connected app:
    ...
    3. The contact email address for Superleap to use if they need to contact you about your connected app.
    4. The logo image URL and icon, if you have one. This is optional.
    5. A brief description to specify what the connected app is for, such as "Application which handles interaction between Superleap and Amazon AppFlow console".
...
To retrieve the client ID and client secret for use in your OAuth flow, you can view your connected app in Superleap by choosing *Apps* and then *App Manager*, and then selecting the connected app that you created.
For more information on connected apps in Superleap, see Connected Apps (https://help.superleap.com/articleView?id=connected_app_overview.html)
in the Superleap documentation.
